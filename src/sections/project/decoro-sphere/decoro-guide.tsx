...
// constants
import { contentMap } from 'src/constants/project';
...
function DecoroGuide({ isOpen, onClick, title, category, hasImage, hasDimensions }: Props) {
  ...
  const stone_ceramic =
    category === 'stone_ceramic_flooring' || category === 'stone_ceramic_wall_covering';
      ...
      <DialogContent sx={{ color: 'text.secondary', pb: 3 }}>
        {stone_ceramic && (
          <Typography variant="body2" mb={2}>
            {t(`project.display_wall_covering_and_flooring`)}
          </Typography>
        )}
        {hasDimensions && (
        ...
        {hasDimensions && (
          <Typography mt={2} variant="body2">
            {t(`project.guide_enter_length_width`)}
          </Typography>
        )}
