...
export default function DevicesView({ title, organization_id, ...other }: Props) {
  const { t } = useLocales();
  ...
  const os = useMemo(
    () => viewData?.data?.items.views.map((item) => item.operating_sys) || [],
    [viewData]
  );

  // Calculate the maximum value for normalization
  const maxProgressValue = useMemo(() => {
    const allValues = serializeDevicesData(os).map((progress) => progress.value);
  ...
  return (
    <Card {...other}>
      ...
      <Stack spacing={3} sx={{ p: 3 }}>
        {os &&
          serializeDevicesData(os).map((progress) => (
