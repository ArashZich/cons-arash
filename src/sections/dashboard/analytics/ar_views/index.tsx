...
export default function ARViews({ title, organization_id, ...other }: Props) {
  ...
  const dataAR = useMemo(
    () => viewData?.data?.items.views.map((item) => item.is_ar) || [],
    [viewData]
  );

  const { categories, data } = serializeBooleanData(
    date as Date[],
    t('analytics.ar_views'),
    dataAR as boolean[],
    isRtl
  );
